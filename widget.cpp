    ...
    , ui(new Ui::Widget)
{
    ...
    QStringList listBuadrate = serial->getCommBps();
    ui->cbx_baudrate->clear();
    ui->cbx_baudrate->addItems(listBuadrate);
    if (listBuadrate.size())
    {
        ui->cbx_baudrate->setCurrentIndex(listBuadrate.size() - 1);
    }
    onSerialChanged();
    ...
    QString upgradename = settings.value("UpgardeFile" , "").toString().trimmed();
    ui->ledit_updatefile->setText(upgradename);
...
void Widget::appendMsgout(QString msg)
{
...
void Widget::onSerialChanged()
{
    quint32 index = 0;
    QList<QSerialPortInfo> listProtInfo = serial->getCommNames();
    ui->cbx_serialnum->clear();
    std::sort(listProtInfo.begin() , listProtInfo.end() , \
    ...
    for (int i = 0; i < listProtInfo.size(); i++)
    {
        ...
        ui->cbx_serialnum->addItem(portName);
        if (listProtInfo[i].description().contains("USB-to-Serial")
            || listProtInfo[i].description().contains("Prolific PL2303GT USB Serial"))
        {
            index = i;
        }
    }
    ui->cbx_serialnum->setCurrentIndex(index);
}

void Widget::onConnectResp(quint8 result)
{
    timerConnect->stop();
    if (0x00 == result)
    {
...
void Widget::onDisConnectResp(quint8 result)
{
    if (0x00 == result)
    {
        onSerialDisconnectedResp();
...
void Widget::onUpdateResp(quint16 result , QByteArray buffer)
{
...
void Widget::onMsgRead(QByteArray buffer)
{
    if (buffer.size() < 10)
    {
        return;
    ...
    switch ((code - 0x8000))
    {
    case (quint16)paramlist::HP10_CONNECT:
        if (buffer.size() > 10)
        {
            onConnectResp(buffer[11]);
        }
        break;
    case (quint16)paramlist::HP10_DISCONNECT:
        if (buffer.size() > 10)
        {
            onDisConnectResp(buffer[11]);
        }
        break;
    case (quint16)paramlist::HP10_UPDATE:
        {
...
void Widget::onSerialDisconnectedResp()
{
    mutex.lock();
    serial->close();
    isConnected = false;
...
void Widget::on_btn_connect_clicked()
{
    ...
    {
        if ("断开连接" == ui->btn_connect->text())
        {
            return;
        }

        QString commName = ui->cbx_serialnum->currentText();
        QList<QSerialPortInfo> listProtInfo = serial->getCommNames();
        foreach(const auto & info , listProtInfo)
        {
            if (commName.contains(info.portName()))
            {
                commName = info.portName();
            }
        }
        qint32 baudrate = serial->getCommBps(ui->cbx_baudrate->currentText());

        bool result = serial->open(commName , baudrate);
        if (!result)
        {
            serial->close();
...
void Widget::on_btn_choice_clicked()
{
    QSettings settings(SETTINGSFILE , QSettings::IniFormat);
    QString upgradename = settings.value("UpgardeFile" , "").toString().trimmed();

    QString s = QFileDialog::getOpenFileName(this , "选择升级文件" , upgradename , "BIN文件(*.bin)");
    if (!s.isEmpty())
    {
        ui->ledit_updatefile->setText(s);
        settings.setValue("UpgardeFile" , s);
    }
}

void Widget::on_btn_upgrade_clicked()
{
    QString filename = ui->ledit_updatefile->text();
    if (filename.isEmpty())
    {
        QMessageBox::information(this , tr("提示") , tr("打开文件失败") , QMessageBox::Ok);
        return;
    }
    ...
    quint32 crc = crc32(buffer , 0);
    QByteArray updateStop(1 , 0x03);
    updateStop.append((crc >> 24) & 0xFF);
    ...
    if (timerUpgrade != nullptr)
    {
        delete timerUpgrade;
        timerUpgrade = nullptr;
    }

    timerUpgrade = new QTimer(this);
    ...
    timerUpgrade->setSingleShot(true);
}
...
quint32 Widget::crc32(QByteArray buffer , quint32 init)
{
    quint32 crc = init;
...
void Widget::enableUI(quint8 enable)
{
    ui->btn_upgrade->setEnabled((enable >> 1) & 0x01);
    ui->btn_connect->setEnabled((enable >> 2) & 0x01);
    ui->btn_choice->setEnabled((enable >> 3) & 0x01);
    ui->ledit_updatefile->setEnabled((enable >> 4) & 0x01);
    ui->cbx_baudrate->setEnabled((enable >> 5) & 0x01);
    ui->cbx_serialnum->setEnabled((enable >> 6) & 0x01);
}
